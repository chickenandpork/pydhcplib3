...
parser.add_option(
    ...
    action="store_true",
    dest="version",
    help="Get pydhcplib3 and pydhcp version.",
    default=False,
)
parser.add_option(
    "-o",
...
def main():
    ...
    if not options.input:
        ...
        print_error(
            "Warning : no input option defined. Default is *stdin* with option *binary*."
        )
    ...
    if not options.output:
        ...
        print_error(
            "Warning : no output option defined. Default is *stdout* with option *binary*."
        )
    ...
    if (
        not options.output
        or options.output[0] == "file"
        or options.output[0] == "stdout"
    ):
        count = 1
...
def process_input(_input, _count):
    ...
    if _type == "device":
        if not option_binary:
            print_error(
                "Warning : option *readable* has no effect for types *device* and *address*"
            )
        if option_up:
        ...
        if option_up:
            print_error("Warning : option *up* has effect only for type *device* ")
        if not option_binary:
            print_error(
                "Warning : option *readable* has no effect for types *device* and *address*"
            )
        ...
        if option_up:
            print_error("Warning : option *up* has effect only for type *device* ")
        if option_binary and _count != 1:
            print_error("Warning : option *count* is always 1 for type *file*")
...
def check_device(device_name, _up):
    ...
    if _up:
        device.setStatusUp(device_name)
    ...
    for each in all_devices:
        if each == device_name:
            valid += 1
    if valid == 0:
        print_error("Warning : no configured device - " + device_name + " - found.")
...
def process_inline_options(definition):
    ...
    try:
        ...
        print_error(
            "Error : unknown type <"
            + input_type
            + "> (device, address, file, input, or output)"
        )
        return False
    ...
    for each in input_options:
        try:
            ...
            print_error(
                "Error : unknown option <" + each + "> (readable, binary, up, or noup)"
            )
            return False
        if each == "up":
            mutual_up += 1
