...
class DhcpPacket(DhcpBasicPacket):
    def str(self):
        ...
        op = self.packet_data[
            dhcp_constants.DhcpFields["op"][0] : dhcp_constants.DhcpFields["op"][0]
            + dhcp_constants.DhcpFields["op"][1]
        ]
        printable_data += (
            "op : " + dhcp_constants.DhcpFieldsName["op"][str(op[0])] + "\n"
        )
        ...
        for opt in [
            ...
            result = ""
            if dhcp_constants.DhcpFieldsTypes[opt] == "int":
                result = str(data[0])
            elif dhcp_constants.DhcpFieldsTypes[opt] == "int2":
                result = str(data[0] * 256 + data[1])
            elif dhcp_constants.DhcpFieldsTypes[opt] == "int4":
            ...
            elif dhcp_constants.DhcpFieldsTypes[opt] == "ipv4":
                ...
                for iterator in range(6):
                    result += [
                        str(hexsym[data[iterator] / 16] + hexsym[data[iterator] % 16])
                    ]
        ...
        for opt in list(self.options_data.keys()):
            ...
            if opt == "dhcp_message_type":
                result = dhcp_constants.DhcpFieldsName["dhcp_message_type"][
                    str(data[0])
                ]
            elif dhcp_constants.DhcpOptionsTypes[optnum] == "char":
                result = str(data[0])
            elif dhcp_constants.DhcpOptionsTypes[optnum] == "16-bits":
                result = str(data[0] * 256 + data[0])
            elif dhcp_constants.DhcpOptionsTypes[optnum] == "32-bits":
                result = str(ipv4(data).int())
                ...
                if optnum == 55:  # parameter_request_list
                    result = ",".join(
                        [dhcp_constants.DhcpOptionsList[each] for each in data]
                    )
                else:
                    result += str(data)
    ...
    def _OptionsToBinary(self, parameter, value):
        ...
        if p == "op" or p == "htype":
            ...
            tmp = []
            for each in value:
                if each in dhcp_constants.DhcpOptions:
                    tmp.append(dhcp_constants.DhcpOptions[each])
            return tmp
        ...
        try:
            option_type = dhcp_constants.DhcpOptionsTypes[
                dhcp_constants.DhcpOptions[parameter]
            ]
        except KeyError:
            return False
