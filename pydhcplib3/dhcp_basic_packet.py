...
class DhcpBasicPacket:
    ...
    def IsDhcpPacket(self):
        if self.packet_data[236:240] != dhcp_constants.MagicCookie:
            return False
        return True
    ...
    def CheckType(self, variable):
        if type(variable) is list:
            for each in variable:
                if (type(each) is not int) or (each < 0) or (each > 255):
                    return False
            return True
        else:
            return False
    ...
    def DeleteOption(self, name):
        ...
        # Set field to 0
        if name in dhcp_constants.DhcpFields:
            begin = dhcp_constants.DhcpFields[name][0]
            end = (
                dhcp_constants.DhcpFields[name][0] + dhcp_constants.DhcpFields[name][1]
            )
    ...
    def SetOption(self, name, value):
        ...
        if name in dhcp_constants.DhcpFields:
            if len(value) != dhcp_constants.DhcpFields[name][1]:
                sys.stderr.write(
                    "pydhcplib3.dhcp_basic_packet.setoption error, bad option length : "
                    + name
                )
                return False
            begin = dhcp_constants.DhcpFields[name][0]
        ...
        # if name is a dhcp option
        elif name in dhcp_constants.DhcpOptions:
            ...
            specs = fields_specs[
                dhcp_constants.DhcpOptionsTypes[dhcp_constants.DhcpOptions[name]]
            ]
            length = len(value)
        ...
        sys.stderr.write(
            "pydhcplib3.dhcp_basic_packet.setoption error : unknown option " + name
        )
        return False
    ...
    def IsOption(self, name):
        if name in self.options_data:
            return True
        elif name in dhcp_constants.DhcpFields:
            return True
        else:
            return False
    ...
    def EncodePacket(self):
        ...
        for each in list(self.options_data.keys()):
            ...
            order[dhcp_constants.DhcpOptions[each]].append(
                dhcp_constants.DhcpOptions[each]
            )
    ...
    def DecodePacket(self, data, debug=False):
        ...
        while (
            self.packet_data[iterator : iterator + 4] != dhcp_constants.MagicCookie
            and iterator < end_iterator
        ):
            iterator += 1
        iterator += 4
        ...
        while iterator < end_iterator:
            if self.packet_data[iterator] == 0:  # pad option
                ...
                self.packet_data[iterator] in dhcp_constants.DhcpOptionsTypes
                and self.packet_data[iterator] != 255
            ):
                opt_len = self.packet_data[iterator + 1]
                opt_first = iterator + 1
                self.options_data[
                    dhcp_constants.DhcpOptionsList[self.packet_data[iterator]]
