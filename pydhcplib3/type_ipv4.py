...
class ipv4:
    def __init__(self, value="0.0.0.0"):
        ...
        elif ip_type is list:
            if not self.CheckNumList(value):
                raise ValueError
            self._ip_numlist = value
            self._NumlistToString()
            self._StringToLong()
        elif ip_type is int:  # python3: long is now an int, just really big
            self._ip_long = value
            self._LongToNumlist()
            self._NumlistToString()
        elif ip_type is bool:
            self._ip_long = 0
            self._LongToNumlist()
            self._NumlistToString()
    ...
    def __cmp__(self, other):
        if self._ip_long == other._ip_long:
            return 0
        elif self._ip_long < other._ip_long:
            return -1
        else:
            return 1
